...
public:
    ...
    TreeNode* util (vector<int>& preorder, vector<int>& inorder, int low, int high, int &pos) {
        ...
        int index = find[preorder[pos]];
        pos++;
        root -> left = util (preorder, inorder, low, index-1, pos);
        root -> right = util (preorder, inorder, index+1, high, pos);
        return root;
    }
    ...
    TreeNode* buildTree(vector<int>& preorder, vector<int>& inorder) {
        ...
        return util(preorder, inorder, 0, preorder.size()-1, pos);
    }
};
